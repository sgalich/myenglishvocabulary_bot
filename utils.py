...
def my_keyboard(text: str) -> ReplyKeyboardMarkup:
...
def inline_keyboard(mini=False) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton(text=texts.B_FLIP, callback_data='flip')],
            [
                InlineKeyboardButton(text=texts.B_REMOVE,
...
def make_inline_keyboard_choose_language(language_code: str
                                         ) -> InlineKeyboardMarkup:
    text = texts.get(language_code)
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(text=text.BUTTON_RUSSIAN,
                                     callback_data='ru'),
                InlineKeyboardButton(text=text.BUTTON_ARABIC,
                                     callback_data='ar')
            ],
            [
                InlineKeyboardButton(text=text.BUTTON_ENGLISH,
                                     callback_data='en'),
            ]
        ],
        resize_keyboard=True
...
def clear(word: str) -> str:
    ...
    try:
        word = word.replace('\n', '').replace('\t', ' ').replace('\r', '') \
            ...
            .replace('&sbquo;', '\"').replace('&bdquo;', '\"').replace(
            '&tilde;', '~') \
            .replace('&circ;', '^').replace('&amp;', '&').replace('&lsaquo;',
                                                                  '‹') \
            .replace('&rsaquo;', '›').replace('&permil;', '‰').replace(
            '&euro;', '€') \
            .strip().lower()
